    ...
    for (i = 1; i < argc; i++) {
        ...
        if ((error = ntfsclone_open(argv[i], (char *)NULL, SYSDEP_OPEN_RO,
            ...
            if (((error = ntfsclone_verify(ntctx)) == 0) || dontcare) {
                ...
                fprintf(stdout,
                        "%s: %" PRId64 " blocks, %" PRIu64
                        " blocks scanned, %" PRIu64 " unset, %" PRIu64
                        " set, %" PRIu64 " strange\n",
                        argv[i], p->nc_head.nr_clusters, bmscanned, unset, set,
                        strange);
                if ((iob =
                         (unsigned char *)malloc(ntfsclone_blocksize(ntctx)))) {
                    int *       fd = (int *)p->nc_fd;
                    off_t       sblkpos;
                    off_t       fsize;
                    struct stat sbuf;
                    ...
                    fprintf(stdout,
                            ...
                            (long long)ntfsclone_blocksize(ntctx),
                            (long long)sblkpos);
                    /*
                    ...
                    for (lastset = p->nc_head.nr_clusters - 1;
                         lastset > 0 &&
                         (bitmap_bit_value(v->v10_bitmap, lastset) == 0);
                         lastset--)
                        ;
                    if ((error = ntfsclone_seek(ntctx, lastset)) == 0) {
                        if ((error = ntfsclone_readblocks(ntctx, iob, 1)) ==
                            0) {
                            off_t cpos, eofpos;
                    ...
                    fprintf(stderr, "%s: cannot malloc %" PRId64 " bytes\n",
                            argv[i], ntfsclone_blocksize(ntctx));
                    anomalies++;
                }
            } else {
